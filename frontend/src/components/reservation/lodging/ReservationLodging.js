...
import ReservationLodgingPopUp from "./ReservationLodgingPopUp";
import ReservationChosenLodging from "./ReservationChosenLodging";
import {useDispatch, useSelector} from "react-redux";
import {
    setReservationArrivalDateLodging,
    setReservationClientsCountLodging, setReservationCommentLodging,
    setReservationDepartureDateLodging
} from "../../../actions/index";
import {add} from "date-fns";


const ReservationLodging = () =>{
    ...
    // Reservation lodging redux data
    const reduxData= useSelector(state => state.reservationConfirmedLodging)
    ...
    //Handlers
    const handleArrivalChange = (newDate) => dispatch(setReservationArrivalDateLodging(newDate))
    const handleDepartureChange = (newDate) => dispatch(setReservationDepartureDateLodging(newDate))
    const handleClientCountChange= (event)=> dispatch(setReservationClientsCountLodging(event.target.value))
    const handleCommentChange= (event)=> dispatch(setReservationCommentLodging(event.target.value))

    // Lodging choosing modal state & handlers
    const [lodgingOpen, setLodgingOpen]= useState(false);
    const handleLodgingOpen= () => setLodgingOpen(true)
    const handleLodgingClose= () => setLodgingOpen(false)


    // Set redux state clientsCount default
    useEffect(()=> {
        dispatch(setReservationClientsCountLodging(clientSelectItems[0]))
    }, [])
    ...
    return(
        <Box sx={{marginBottom: '100px'}}>
            <ReservationLodgingPopUp
                open={lodgingOpen} handleClose={handleLodgingClose}/>
            <form className=" space-y-3">
                <Box className="text-center" >
                    {
                        // If the uses already chose a lodging => show it
                        reduxData.lodging.id?
                            <Box className="mb-4">
                            <ReservationChosenLodging data={reduxData.lodging} /><br/>
                            </Box>:
                            //Else show this
                            <label className="mr-5 text-lg">You can choose a lodging</label>}

                    <Button variant="contained"
                            className="w-[120px] h-[50px]"
                            sx={chooseButtonSx}
                            onClick={handleLodgingOpen}
                          >{reduxData.lodging.id?'Change':'Choose'}</Button>
                </Box>
                <Box className="flex">
...
export default ReservationLodging;