...
import { getSingleTransport, clearErrors, newReview } from '../../actions/transportActions'
...
//////////////////
    ...
    const { loading, error, transport } = useSelector(state => state.transportDetails)
...
<h3 style={{fontFamily: "normal",fontWeight: 700, fontSize: "32px",
 lineHeight: "39px", margin: "20px 290px 0", color:"#DA1D6C"}}>{transport.name}</h3>
            <div className="container-slider">
       {transport.images && transport.images.map((image , index) => (
                    <div
                    key={image.public_id}
                    className={slideIndex === index + 1 ? "slide active-anim" : "slide"}
                    >
                        <img className="d-block w-100" src={image.url} alt={transport.name}/>
                    </div>
            ))}
...
fontSize: "28px",
textAlign:"center",
lineHeight: "39px"}}>{transport.pricepernight} DT / Night</h4>
<br/>
