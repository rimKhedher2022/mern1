...
import { getSingleLodging, clearErrors, newReview } from '../../actions/lodgingActions'
...
//////////////////
    ...
    const { loading, error, lodging } = useSelector(state => state.lodgingDetails)
...
<h3 style={{fontFamily: "normal",fontWeight: 700, fontSize: "32px",
 lineHeight: "39px", margin: "20px 290px 0", color:"#DA1D6C"}}>{lodging.title}</h3>
            <div className="container-slider">
       {lodging.images && lodging.images.map((image , index) => (
                    <div
                    key={image.public_id}
                    className={slideIndex === index + 1 ? "slide active-anim" : "slide"}
                    >
                        <img className="d-block w-100" src={image.url} alt={lodging.title}/>
                    </div>
            ))}
...
letterSpacing: "0.02em",}}>{lodging.description}</p>
        </Paper>
        </Box>
        <Box
      sx={{
...
fontSize: "28px",
textAlign:"center",
lineHeight: "39px"}}>{lodging.pricepernight} DT / Night</h4>
<br/>
